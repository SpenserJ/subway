...
module.exports = function(socket, connections) {
  ...
  socket.on('connect', function(data) {
    var connection;
    if(current_user){
      connection = connections[current_user.username][0];
    }
    if(connection === undefined) {
      ...
      if(current_user){
        ...
        conn.save();
        if (typeof connections[current_user.username] === 'undefined')
          connections[current_user.username] = [];
        connections[current_user.username].push(connection);
      }
    } else {
    ...
    socket.on('disconnectServer', function() {
      ...
      if(current_user) {
        for (var i = 0; i < connections[current_user.username].length; i++)
          connections[current_user.username][i].disconnect();
